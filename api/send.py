...
def unlockAccount(address=None, password="vasainc..", duration=3600):
    """
...
def initialize(contractTx_blockNumber=797, contractTx_transactionIndex=0):
    """
...
def contract_set_via_web3(contract, pubKey,userHash,metadataHash,userClass,isOrg, privateFor=PRIVATE_FOR, gas=90000000):
    """
    call the .set(arg) method, possibly with 'privateFor' tx-property
    using the web3 method 
    """

    getData = contract.functions.createAccount(pubKey,userHash,metadataHash,userClass,isOrg).buildTransaction({'gasPrice': 21000000000});
    getData['from'] = w3.toChecksumAddress("0x30f28686aef33adbfbc13797b1d9f5a2f2759f56")
    txParameters = getData
...
def many_transactions(contract, howMany):
    ...
    for i in range(howMany):
        ...
        userHash = "kkkkkkkkk"
        metadataHash = "vasa"
        userClass = "vasa@towardsblockchain.com"
        isOrg = True
        
        #string pubKey, string userHash, string metadataHash, string userClass, bool isOrg
        tx = contract_set(contract,pubKey,userHash,metadataHash,userClass,isOrg)
...
def many_transactions_threaded(contract, howMany):
    ...
    for i in range(howMany):
        t = Thread(target = contract_set,
                   args   = (contract,pubKey,userHash,metadataHash,userClass,isOrg))
        threads.append(t)
        print (".", end="")
